...
def evaluate(
    ...
    @jax.jit
    def _eval_q(agent, obs, act):
        return agent._score_actions(obs[None], act[None])[0]
    ...
    for i in trange(num_eval_episodes + num_video_episodes):
        ...
        while not done:
            ...
            if len(action_queue) == 0:
                ...
                full_action_for_q = action
                action = np.array(action).reshape(-1, action_dim)
                action_chunk_len = action.shape[0]
                for a in action:
                    action_queue.append(a)
                q_val = float(_eval_q(agent, jnp.asarray(observation), jnp.asarray(full_action_for_q)))
                traj_q_values.append(q_val)
            else:
        ...
        if i < num_eval_episodes:
            if len(traj_q_values) > 0:
                add_to(stats, {"mean_q": np.mean(traj_q_values)})
