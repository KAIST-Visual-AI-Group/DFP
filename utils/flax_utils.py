...
def _restore_params_only(agent, loaded_agent_state):
    """Overlay only network params from loaded state, keep current opt_state/step.

    Used when the optimizer has changed (e.g. switching to multi_transform with a
    separate actor LR), so the saved opt_state structure no longer matches.
    """
    current = flax.serialization.to_state_dict(agent)
    if 'network' in loaded_agent_state and 'params' in loaded_agent_state['network']:
        current['network']['params'] = loaded_agent_state['network']['params']
    return flax.serialization.from_state_dict(agent, current)


def restore_agent_with_file(agent, file_path, params_only=False):
    """Just like restore_agent() but expect file_path to include restore_epoch.

    If params_only=True, only network params are restored; opt_state/step keep
    their freshly-initialized values. Use this when the optimizer config
    differs from the saved checkpoint.

    Keys present in the agent but absent from the checkpoint (e.g. newly-added
    modules like actor_drift_ema) are filled with the agent's freshly-
    initialized values so older checkpoints remain loadable.
    """
    assert os.path.exists(file_path), f'File {file_path} does not exist'
    with open(file_path, 'rb') as f:
        load_dict = pickle.load(f)

    # --- Graceful restore: fill missing keys from agent's initialized state ---
    agent_state = flax.serialization.to_state_dict(agent)

    def _fill_missing(target, source):
        """Return source with keys from target added when missing (recursive)."""
        ...
        for k, v in target.items():
            if k not in merged:
                merged[k] = v
                print(f"  [restore] missing key in ckpt: {k!r} — using initialized value")
            else:
                merged[k] = _fill_missing(v, merged[k])
        return merged

    ckpt_agent = _fill_missing(agent_state, load_dict['agent'])
    # ------------------------------------------------------------------------

    if params_only:
        agent = _restore_params_only(agent, ckpt_agent)
    else:
        agent = flax.serialization.from_state_dict(agent, ckpt_agent)

    print(f'Restored from {file_path} (params_only={params_only})')
