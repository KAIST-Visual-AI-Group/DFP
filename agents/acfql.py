...
class ACFQLAgent(flax.struct.PyTreeNode):
    ...
    def critic_loss(self, batch, grad_params, rng):
        ...
        next_actions = self.sample_actions(batch['next_observations'][..., -1, :], rng=sample_rng, use_q_bon=True)
    ...
    @functools.partial(jax.jit, static_argnames=("use_q_bon",))
    def sample_actions(
        self,
        observations,
        rng=None,
        ...
        if self.config["actor_type"] == "distill-ddpg":
            ...
            bon_samples = self._get_bon_samples(use_q_bon=use_q_bon)
            ...
            noises = jax.random.normal(
                ...
                (
                    *observations.shape[: -len(self.config['ob_dims'])],  # batch_size
                    bon_samples, action_dim
                ),
            )
            observations = jnp.repeat(observations[..., None, :], bon_samples, axis=-2)
            ...
            bsize = len(indices)
            actions = jnp.reshape(actions, (-1, bon_samples, action_dim))[jnp.arange(bsize), indices, :].reshape(
                bshape + (action_dim,))
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            ...
            q_alpha=1.0,  # Coefficient for actor q_loss term.
            ...
            q_bon=-1,  # if > 0, overrides actor_num_samples for critic target actions.
            eval_bon=-1,  # if > 0, overrides actor_num_samples for eval rollout actions.
