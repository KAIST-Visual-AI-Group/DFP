...
def make_env(env_name, seed=0):
    ...
    # Convert old controller format to new composite controller format if needed
    if "controller_configs" in env_meta["env_kwargs"]:
        # Use robosuite's built-in refactoring function for backward compatibility
        # This handles conversion from robosuite <= 1.4.1 format to >= 1.5 format
        env_meta["env_kwargs"]["controller_configs"] = refactor_composite_controller_config(
            env_meta["env_kwargs"]["controller_configs"],
            robot_type="Panda",  # The datasets use Panda robot
            arms=["right"]  # Panda has a right arm
        )
